...
###############################################################################
"""
Function to calculate Info(T) or SplitInfo(Attribute) of given dataframe
    
    Parameters: dataframe - targeted dataframe
                column - default value is NURSERY to calculate Info(T)
                         targeted attribute to calculate SplitInfo(Attribute)

    Return: result of Info(T)
"""     
def calculate_Info_T_or_SplitInfo_A(dataframe, column = 'NURSERY'):
    entropy = 0 # to store calculation result 
    list_different_values = get_all_values(column, df_train) # different values of given column
    list_all_values = list(dataframe[column]) # all the values of given column in dataframe
    
    # simply count apprearance frequency of each value in given column in order to calculate p
    for value in list_different_values:
        p = list_all_values.count(value)/len(list_all_values)
        if p == 0:
            continue
        else:
            entropy += -p*math.log(p,2)
...
Function to calculate Gain(Attribute,T) of given dataframe
    ...
    Return: result of Gain(Attribute,T) = (Info(T) - Info(Attribute,T))/SplitInfo(Attribute)
"""      
def calculate_Gain_A_T(atr, dataframe):
	Info_T = calculate_Info_T_or_SplitInfo_A(dataframe)
	Info_A_T = calculate_Info_A_T(atr, dataframe)
	SplitInfo_A = calculate_Info_T_or_SplitInfo_A(dataframe, atr)
	return (Info_T - Info_A_T)/SplitInfo_A
...
def train_c45(dataframe, node):
    ...
    if node.not_leaf and len(dataframe) != 0 and len(list(dataframe.keys())) > 1:
        ...
        for atr in list_atr[:len(list_atr)-1]:
            list_Gain[atr] = calculate_Gain_A_T(atr, dataframe)

        # get the attribute with highest Gain(Attribute, T) value
        max_Gain = max([v for _,v in list_Gain.items()])
        max_Gain_atr = [k for k,v in list_Gain.items() if v == max_Gain]
        ...
        for value, df in group_by_max_atr:
            ...
            if calculate_Info_T_or_SplitInfo_A(df) == 0:
            ...
            # if not, continue perform training for this children node
                ...
                train_c45(df, node.children[value])
...
df_test.to_excel('Prediction_c45.xlsx')
print(df_test)
